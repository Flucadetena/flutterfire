...
grant them full access to your application (If this is your case you may prefer the `Email/Link` authentication method below).
...
## Email/Link Registration & Sign-in

Email/Link is a simpler way of registering users into and app that automatically verifies their email. This requires the user to provide an email address
where a link is send. When clicked the link will open the app and verify that user. With this authentication system Users are registered and/or Sign-in at the same time, with the `emailLinkSignIn()` method.

To use this authentication method, make sure that on Firebase Console under "authetication > email/password" you have "Send link through email" switched on.

### Sending the Email

To create a new account or to sign-in an existing user on your Firebase project call the `sendSignInLinkToEmail()` method with the user's email address and the url of your deepLink, wich you can find in the "Dynamic Links" section in the Firebase Console.
...
The only required parameters are `email:` and `actionCodeSettings: ActionCodeSettings(url:)`. For all the other parameters, you can learn more about them here [ActionCodeSettings](https://firebase.google.com/docs/auth/web/passing-state-in-email-actions?hl=es-419#passing_statecontinue_url_in_email_actions)

### Reciving the dynamic link

First make sure you add [firebase_dynamic_links](https://pub.dev/packages/firebase_dynamic_links) to your pubspec.yaml. And that you have set-up "Dynamic Links" on Firebase Console.

Now we can handle any link coming from your dynamic link url. Then you can handle the authentication link calling the `emailLinkSignIn()` method.

Make sure to call `handleDynamicLinks()` in the build method of "MyApp" Widget.
...
void _handleDeepLink(PendingDynamicLinkData data) async {
  ...
  if (deepLink != null && deepLink.pathSegments.contains('auth')) {
      await AuthService().emailLinkSignIn(deepLink);
  }
}
```
